"""Integration tests using the fixture files."""
...
def _run_comparison(input_path: Path, output_path: Path):
    """Run the full pipeline on two files and return the document score."""
...
def test_good_output_has_coverage():
    """Good output should have high concept coverage."""
    score = _run_comparison(
        FIXTURE_DIR / "input.md", FIXTURE_DIR / "expected-good.md"
    )
    assert score.coverage >= 0.8, f"Coverage too low: {score.coverage}"


def test_ablated_output_has_coverage():
    """Ablated output should also have coverage (meaning preserved)."""
    score = _run_comparison(
        FIXTURE_DIR / "input.md", FIXTURE_DIR / "expected-ablated.md"
    )
    # TF-IDF is purely lexical, so heavy vocabulary replacement drops some
    # concepts below the match threshold. Coverage >= 0.5 confirms that
    # meaning is still partially recoverable even after severe ablation.
    assert score.coverage >= 0.5, f"Coverage too low: {score.coverage}"


def test_good_output_higher_lexical_overlap():
    """Good output should have higher lexical overlap than ablated output."""
