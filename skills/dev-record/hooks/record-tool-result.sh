...
TOOL_RESPONSE=$(printf '%s' "$INPUT" | jq '.tool_response // {}')
TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)
CONTENT=$(printf '%s' "$TOOL_RESPONSE" | jq -r '.content // ""')
...
# Atomic append: write events to temp file, then flock + cat >> to serialize.
LOCK_FILE="$LOG_DIR/.lock_${SESSION_ID}"
TMP_FILE=$(mktemp "$LOG_DIR/.tmp.XXXXXX")
trap 'rm -f "$TMP_FILE"' EXIT

# Record the tool result
SUCCESS=$(printf '%s' "$TOOL_RESPONSE" | jq -r 'if .success == false then "false" else "true" end')
...
if [ "$TOOL_NAME" = "Bash" ]; then
  if printf '%s' "$CONTENT" | grep -qF "No such file or directory"; then
...
# Serialize the append with flock
